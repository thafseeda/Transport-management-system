...
const CarReportList = () => {
  ...
  const [editData, setEditData] = useState({ regNo: '', carName: '', hours: '' });
  ...
  const handleEdit = (report) => {
    ...
    setEditData({ regNo: report.regNo, carName: report.carName, hours: report.hours });
  };
  ...
  const handleEditSave = async (id) => {
    if (!editData.regNo || !editData.carName || !editData.hours || isNaN(editData.hours)) return;
    ...
    if (res.ok) {
      ...
      setEditId(null);
      setEditData({ regNo: '', carName: '', hours: '' });
    }
  };
  ...
  return (
    <div className="busreport-container">
      <h2 className="busreport-title">Car Reports List</h2>
      <button className="busreport-add-btn" style={{marginBottom: '1.5rem'}} onClick={() => navigate('/car')}>Add Reports</button>
      {loading ? <p>Loading...</p> : (
              ...
              <tr key={report._id || idx}>
                {editId === report._id ? (
                  <>
                    ...
                    <td><input name="carName" className="busreport-input" value={editData.carName} onChange={handleEditChange} /></td>
